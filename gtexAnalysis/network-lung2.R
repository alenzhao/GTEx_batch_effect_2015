...
datExpr <- as.data.frame(t(assay(rld2))) # NOTE: rows=samples, columns=genes
colnames(datExpr) <- gtab2$Description
nGenes <- ncol(datExpr)
...
pdf(file = "lung2-sampleClustering.pdf", width = 12, height = 9)
par(cex = 0.6)
par(mar = c(0,4,2,0))
...
# Plot the results:
pdf(file = "lung2-softThreshold.pdf", width = 9, height = 5)
par(mfrow = c(1,2))
cex1 = 0.9
...
pdf(file = "lung2-net.pdf", width = 12, height = 6)
...
pdf("lung2-METree.pdf", width=7, height=6)
plot(METree, main="Clustering of module eigengenes", xlab="", sub="")
dev.off()

## heatmap: eigengenes vs. samples
pdf("lung2-heatmap-eigengenes.pdf", width=15, height=5)
...
pdf("lung2-heatmap-genes.pdf", width=15, height=10)
...
pdf("lung2-death-ventilator.pdf", width=6, height=12)
par(cex = 0.9)
...
annotation_col <- data.frame(MEturquoise,ventilator)
ann_colors <- list(MEturquoise=colorRampPalette(rev(brewer.pal(n = 7, name ="RdYlBu")))(100))
pheatmap(t(datExpr.normed[order(MEturquoise),moduleColors=="turquoise"]), scale="none", cluster_rows=TRUE, show_rownames=TRUE, cluster_cols=FALSE, show_colnames=FALSE, annotation_col=annotation_col, annotation_colors=ann_colors, filename="lung2-pheatmap-ventilator-turquoise.pdf", width=6, height=12, fontsize_row = 4, fontsize = 6)
# pheatmap(t(MEs))
...
pdf("lung2-death-other.pdf", width=6, height=6)
...
# pheatmap(t(datExpr.normed[order(MEblue),moduleColors=="blue"]), scale="none", cluster_rows=TRUE, show_rownames=TRUE, cluster_cols=FALSE, show_colnames=FALSE, annotation_col=annotation_col, annotation_colors=ann_colors, filename="lung2-pheatmap-fast-blue.pdf", width=6, height=6)
...
# Plot the relationships among the eigengenes and the trait
pdf("lung2-gender.pdf", width=6, height=12)
par(cex = 0.9)
...
gender[gender==1] <- "M"
gender[gender==0] <- "F"
annotation_col<- data.frame(MEblack,gender)
ann_colors <- list(MEblack=colorRampPalette(rev(brewer.pal(n = 7, name ="RdYlBu")))(100))
pheatmap(t(datExpr.normed[order(MEblack),moduleColors=="black"]), scale="none", cluster_rows=TRUE, show_rownames=TRUE, cluster_cols=FALSE, show_colnames=FALSE, annotation_col=annotation_col, annotation_colors=ann_colors, filename="lung2-pheatmap-gender-black.pdf", width=6, height=6)
...
# Plot the relationships among the eigengenes and the trait
pdf("lung2-age.pdf", width=6, height=12)
par(cex = 0.9)
...
# Plot the relationships among the eigengenes and the trait
pdf("lung2-age-binary.pdf", width=6, height=12)
par(cex = 0.9)
plotEigengeneNetworks(MET, "Age and eigengenes", marDendro = c(0,4,2,2), marHeatmap = c(6,6,2,2), cex.lab = 0.8, xLabelsAngle = 90, printAdjacency = TRUE)
dev.off()
...
# sizeGrWindow(9,9)
pdf("lung2-network-heatmap.pdf", height=9, width=9)
heatmap(plotTOM)
...
# sizeGrWindow(9,9)
pdf("lung2-network-heatmap-selected.pdf")
...
#                        numericLabels = TRUE, pamRespectsDendro = FALSE,
#                        saveTOMs = TRUE,
#                        saveTOMFileBase = "lung2TOM-blockwise", 
#                        verbose = 3))

###########################################
## save
save(t,bwnet,datExpr,file="lung2-bwnet.rda")
###########################################

## reload
load("lung2-bwnet.rda")
...
# sizeGrWindow(6,6)
pdf(file = "lung2-bwnet.pdf", width = 12, height = 6)
