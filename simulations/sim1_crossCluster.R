...
crossCor <- 0.35 #cross cluter correlation
...
for (i in 1:(nClusters-1)){
  sigma[sum(clusterSize[1:i]),sum(clusterSize[1:i])+1] <- crossCor
  sigma[sum(clusterSize[1:i])+1,sum(clusterSize[1:i])] <- crossCor
}
index.cross <- which((sigma-sigma0)!=0)

## true network
true.net <- sigma*0
true.net[sigma!=bgCor] <- 1
diag(true.net) <- 0

## multivariate normal
set.seed(999)
datExpr <- rmnorm(n=nSamples, varcov=sigma) # NOTE: rows=samples, columns=genes
...
pdf(file = "wgcna-net-sim1.pdf", width = 12, height = 6)
...
##     = sum(abs(estimated edge weights - true edge weights (i.e. sigma)))/N, where N=nGenes^2
mae.wgcna <- sum(abs(est.wgcna-sigma))/(nGenes^2)
mae.wgcna
## binary
mae.wgcna.bin <- sum(abs(est.wgcna.bin-true.net))/(nGenes^2)
mae.wgcna.bin
## cross cluster edges
mae.wgcna.cross <- sum(abs(est.wgcna.bin-true.net)[index.cross])/length(index.cross)
mae.wgcna.cross
...
## MAE
mae.aracne <- sum(abs(est.aracne-sigma))/(nGenes^2)
mae.aracne
## binary
mae.aracne.bin <- sum(abs(est.aracne.bin-true.net))/(nGenes^2)
mae.aracne.bin
## cross cluster edges
mae.aracne.cross <- sum(abs(est.aracne.bin-true.net)[index.cross])/length(index.cross)
mae.aracne.cross
...
## MAE
mae.bn <- sum(abs(est.bn-sigma))/(nGenes^2)
mae.bn
## binary
mae.bn.bin <- sum(abs(est.bn.bin-true.net))/(nGenes^2)
mae.bn.bin
## cross cluster edges
mae.bn.cross <- sum(abs(est.bn.bin-true.net)[index.cross])/length(index.cross)
mae.bn.cross
...
Niter <- 1000
mae.wgcna <- mae.wgcna.bin <- mae.wgcna.cross <- mae.aracne <- mae.aracne.bin <- mae.aracne.cross <- mae.bn <- mae.bn.bin <- mae.bn.cross <- vector()
...
for (iter in 1:Niter){
  ...
  est.wgcna.bin <- est.wgcna>bin.thred
  ## MAE
  mae.wgcna[iter] <- sum(abs(est.wgcna-sigma))/(nGenes^2)
  ## binary
  mae.wgcna.bin[iter] <- sum(abs(est.wgcna.bin-true.net))/(nGenes^2)
  ## cross cluster edges
  mae.wgcna.cross[iter] <- sum(abs(est.wgcna.bin-true.net)[index.cross])/length(index.cross)
  ...
  est.aracne.bin <- est.aracne>bin.thred
  ## MEA
  mae.aracne[iter] <- sum(abs(est.aracne-sigma))/(nGenes^2)
  ## binary
  mae.aracne.bin[iter] <- sum(abs(est.aracne.bin-true.net))/(nGenes^2)
  ## cross cluster edges
  mae.aracne.cross[iter] <- sum(abs(est.aracne.bin-true.net)[index.cross])/length(index.cross)
  
  ## BAYSIAN: Hill climbing greedy search
  bn.hc <- hc(datExpr)
  ## find linear coeficients from bn.hc
  bn.hc.fit <- bn.fit(bn.hc, datExpr)
  coefs <- coef(bn.hc.fit)
  ...
  est.bn.bin <- est.bn>bin.thred
  ## MAE
  mae.bn[iter] <- sum(abs(est.bn-sigma))/(nGenes^2)
  ## binary
  mae.bn.bin[iter] <- sum(abs(est.bn.bin-true.net))/(nGenes^2)
  ## cross cluster edges
  mae.bn.cross[iter] <- sum(abs(est.bn.bin-true.net)[index.cross])/length(index.cross)
...
mean(mae.wgcna);mean(mae.wgcna.bin);mean(mae.wgcna.cross)
mean(mae.aracne);mean(mae.aracne.bin);mean(mae.aracne.cross)
mean(mae.bn);mean(mae.bn.bin);mean(mae.bn.cross)
...
pdf("roc_wgcna_sim1.pdf", width=6, height=6)
...
pdf("roc_aracne_sim1.pdf", width=6, height=6)
...
pdf("roc_bn_sim1.pdf", width=6, height=6)
...
summary(as.vector(est.wgcna.all))
summary(as.vector(est.aracne.all))
summary(as.vector(est.aracne.all)[est.aracne.all!=0])
summary(as.vector(abs(est.bn.all)))
summary(as.vector(abs(est.bn.all[est.bn.all!=0])))
